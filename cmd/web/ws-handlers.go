...
func (app *application) WsEndPoint(w http.ResponseWriter, r *http.Request) {
	...
	printConnect := fmt.Sprintf("Client connected from %s", r.RemoteAddr)
	app.infoLog.Println(printConnect)
...
func (app *application) ListenToWsChannel() {
	...
	for {
		...
		case "deleteUser":
			...
			app.broadcaseToAll(response)
		default:
		}
	}
}
func (app *application) broadcaseToAll(response WsJsonResponse) {
